...
################################################################################
#
# Purpose: Use a Debian 9 VM to create a monitoring/quorum/voting/micro node
#
# Assumptions: Debian 9 (stretch) installed with "standard" package selection 
...
sed -i "s/main contrib/main non-free contr "standard" package selection ib/g" /etc/apt/sources.list
...
apt remove -y os-prober resolvconf rdnssd

## Update
apt-get -y dist-upgrade 

# Install Proxmox
apt-get install --force-yes -y proxmox-ve postfix open-iscsi
