...
#
# Assumes 2 or 4 identical disks at /dev/nvme0n1 and /dev/nvme1n1,nvme2n1,nvme3n1,sde,sdf it ignores any extra disks which are not identical
# Will make sure the raid 1 use nvme0n1 and the next identical sized disk, eg. nvme2n1 if nvme1n1 is not the same siza as nvme0n1
# software raid 1 (mirror) will be setup as well as LVM and will automatically detect and set the swap size
# If 4 identical disks are detected (nvme0n1,nvme1n1,nvme2n1,nvme3n1) raid 10 will be used. (mirror and striped)
#
...
elif [[ $(awk '/nvme0n1$/{printf "%i", $(NF-1) / 1000 / 1000}' /proc/partitions) -eq $(awk '/nvme2n1$/{printf "%i", $(NF-1) / 1000 / 1000}' /proc/partitions) ]]; then
  MY_RAID_ENABLE="yes"
...
if [ "$MY_RAID_ENABLE" == "yes" ]; then
  if [[ $(awk '/nvme0n1$/{printf "%i", $(NF-1) / 1000 / 1000}' /proc/partitions) -eq $(awk '/nvme1n1$/{printf "%i", $(NF-1) / 1000 / 1000}' /proc/partitions) ]] && [[ $(awk '/nvme0n1$/{printf "%i", $(NF-1) / 1000 / 1000}' /proc/partitions) -eq $(awk '/nvme2n1$/{printf "%i", $(NF-1) / 1000 / 1000}' /proc/partitions) ]] && [[ $(awk '/nvme0n1$/{printf "%i", $(NF-1) / 1000 / 1000}' /proc/partitions) -eq $(awk '/nvme3n1$/{printf "%i", $(NF-1) / 1000 / 1000}' /proc/partitions) ]] ; then
    MY_RAID_LEVEL="10"
    MY_RAID_SLAVE=",nvme1n1,nvme2n1,nvme3n1"
  else
    MY_RAID_LEVEL="1"
  fi
  echo "RAID ENABLED"
...
if [ "$MY_SLOG" == "0" ] ; then
  ...
  MY_SLOG=""
elif [ "$(cat /sys/block/nvme1n1/queue/rotational)" == "1" ] || [ "$(cat /sys/block/nvme2n1/queue/rotational)" == "1" ]  || [ "$(cat /sys/block/nvme3n1/queue/rotational)" == "1" ] || [ "$(cat /sys/block/sde/queue/rotational)" == "1" ] || [ "$(cat /sys/block/sdf/queue/rotational)" == "1" ] ; then
